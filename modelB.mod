...
set INTERSECCIONS_VEINES within {INTERSECCIONS, INTERSECCIONS}; # parella d'identificadors de dues interseccions que son veines (es troben a menys de 300m l'una de l'altre)
...
subject to distancia_minima_sensor{(i,j) in INTERSECCIONS_VEINES}: s_i[i] + s_i[j] <= 1; # Les parelles d'interseccions veines no poden tenir sensors alhora